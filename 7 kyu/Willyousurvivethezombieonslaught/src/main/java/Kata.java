...
public class Kata {
    ...
    public static String zombieShootout(int zombies, int range, int ammo) {
        if (zombies == ammo && zombies == range * 2) return "You shot all " + zombies + " zombies.";
        int countZ = 0;
        float r = range;

        while (zombies > 0 && r > 0 && ammo > 0) {
            zombies--;
            r -= 0.5;
            ammo--;
            countZ++;
        ...
        if (zombies < 1 && r > 0) {
            return "You shot all " + countZ + " zombies.";
        }
        if ((zombies > 0 && ammo > 0) || (r == 0 && zombies > 1 && ammo == 0)) {
            return "You shot " + countZ + " zombies before being eaten: overwhelmed.";
        }
        if (ammo < 1 && zombies >= 1) {
            return "You shot " + countZ + " zombies before being eaten: ran out of ammo.";
        }
        if (zombies == ammo) return "You shot " + zombies + " zombies before being eaten: overwhelmed.";
        return "You shot all " + countZ + " zombies.";
    }
}